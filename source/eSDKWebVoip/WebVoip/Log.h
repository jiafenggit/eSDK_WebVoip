	...
	~CLogTrace()
	{
		try 
		{
			...
			if (TRACE_INFO == m_Level)
			{
				...
				Log_Interface_Info("CC", "1", "COM", m_Func, "", "", "", reqTime, rspTime, "", m_Msg);
			}
		} 
		catch(...){} ;
...
#define LOG_DEBUG(fmt, ...)	Log_Run_Debug(PRODUCT_NAME,FormatedMsg(fmt, ##__VA_ARGS__).c_str())
#define LOG_INFO(fmt, ...)	Log_Run_Info(PRODUCT_NAME,FormatedMsg(fmt, ##__VA_ARGS__).c_str())
#define LOG_WARN(fmt, ...)	Log_Run_Warn(PRODUCT_NAME,FormatedMsg(fmt, ##__VA_ARGS__).c_str())
#define LOG_ERROR(fmt, ...)	Log_Run_Error(PRODUCT_NAME,FormatedMsg(fmt, ##__VA_ARGS__).c_str())
//lint +e438
